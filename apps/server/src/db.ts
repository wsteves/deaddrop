...
export function createJob(data: any, postedBy: string) {
  ...
  const stmt = db.prepare(`
    INSERT INTO jobs (
      id, title, slug, description, shortDescription, requirements, responsibilities, benefits,
      companyId, postedBy, salary, salaryMin, salaryMax, salaryCurrency, equityMin, equityMax,
      employmentType, experienceLevel, remote, location, timezone, skills, tags, techStack,
      applicationMethod, applicationEmail, applicationUrl, applicationInstructions,
      priority, isSponsored, isFeatured, isRemoteFriendly, category, department,
      verificationStatus, publishedAt, expiresAt, createdAt, updatedAt
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);

  const slug = data.title?.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || null;

  stmt.run(
    id, data.title, slug, data.description, data.shortDescription, data.requirements, data.responsibilities, data.benefits,
    data.companyId, postedBy, data.salary, data.salaryMin, data.salaryMax, data.salaryCurrency || 'EUR', data.equityMin, data.equityMax,
    data.employmentType, data.experienceLevel, data.remote, data.location, data.timezone, 
    JSON.stringify(data.skills || []), JSON.stringify(data.tags || []), JSON.stringify(data.techStack || []),
    data.applicationMethod || 'email', data.applicationEmail, data.applicationUrl, data.applicationInstructions,
    data.priority || 'normal', data.isSponsored ? 1 : 0, data.isFeatured ? 1 : 0, data.isRemoteFriendly ? 1 : 0, 
    data.category, data.department, 'published', now, data.expiresAt, now, now
  );
